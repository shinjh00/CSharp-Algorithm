namespace _040._Queue
{
    internal class Program
    {
        ...
        // 2. 전단 & 후단
        // 삽입 & 삭제 시 데이터를 앞당기지 않고 front와 rear를 표시하여 삽입할 위치와 삭제할 위치를 지정
        //
        // - 삽입 -
        // rear 위치에 데이터를 추가하고 rear를 한칸 뒤로 이동
        //     f       r                f         r
        //     ↓       ↓                ↓         ↓      
        //  ┌─┬─┬─┬─┬─┬─┬─┬─┐        ┌─┬─┬─┬─┬─┬─┬─┬─┐
        //  │ │2│3│4│5│ │ │ │   =>   │ │2│3│4│5│6│ │ │
        ...
        //
        // - 삭제 -
        // front 위치에 데이터를 추가하고 front를 한칸 뒤로 이동
        //     f         r                f       r
        //     ↓         ↓                ↓       ↓
        //  ┌─┬─┬─┬─┬─┬─┬─┬─┐        ┌─┬─┬─┬─┬─┬─┬─┬─┐
        //  │ │2│3│4│5│6│ │ │   =>   │ │ │3│4│5│6│ │ │
        ...
        //
        // - 문제발생 -
        // rear가 front를 침범하는 경우 모든 공간이 비어있는 상황과 가득차 있는 상황을 구분할 수 없음
        // 
        //         rf                       rf       
        //         ↓↓                       ↓↓       
        // ┌─┬─┬─┬─┬─┬─┬─┬─┐        ┌─┬─┬─┬─┬─┬─┬─┬─┐
        ...
        // 4. 포화상태확인
        // front와 rear가 일치하는 경우를 비어있는 경우로 판정
        // rear가 front 전위치에 있는 경우를 실제로는 한자리가 비어있지만 가득찬 경우로 판정
        //         rf                      r f       
        //         ↓↓                      ↓ ↓       
        // ┌─┬─┬─┬─┬─┬─┬─┬─┐        ┌─┬─┬─┬─┬─┬─┬─┬─┐
        // │ │ │ │ │ │ │ │ │        │5│6│7│ │1│2│3│4│
        ...
        // queue.Enqueue(i);       // rear에 i를 삽입
        // queue.Peek();           // front 데이터 반환
        // queue.Dequeue();        // front 데이터부터 삭제
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                queue.Enqueue(i);                       // 입력순서 : 0, 1, 2, 3, 4           큐 : 0, 1, 2, 3, 4
            }
            ...
            for (int i = 0; i < 3; i++)
            {
                Console.WriteLine(queue.Dequeue());     // 출력순서 : 0, 1, 2                 큐 : 3, 4
            }
            ...
            for (int i = 5; i < 10; i++)
            {
                queue.Enqueue(i);                       // 입력순서 : 5, 6, 7, 8, 9           큐 : 3, 4, 5, 6, 7, 8, 9
            }
